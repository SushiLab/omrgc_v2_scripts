...
# Load data
metaG<-fread("zcat < ../data/raw/OM-RGC_v2_release_profile_by_KO_metaG.tsv.gz",sep="\t",header=T,data.table = F)
rownames(metaG)<-metaG$KO # Move KO variale to rownames
metaG<-metaG[,-1]
...
# Load data
metaT<-fread("zcat < ../data/raw/OM-RGC_v2_release_profile_by_KO_metaT.tsv.gz",sep="\t",header=T,data.table = F)
rownames(metaT)<-metaT$KO # Move KO variale to rownames
metaT<-metaT[,-1]
...
metaG.norm<-metaG/apply(metaG[,match(cog.10MGs$V2,colnames(metaG))],1,median)
metaT.norm<-metaT/apply(metaT[,match(cog.10MGs$V2,colnames(metaT))],1,median)
...
p2<-ggplot(toplot.merged %>% filter(ko!="unknown") %>% arrange(mean),aes(x=mean,y=variance)) +
  ...
  theme_bw() +
  ylim(limits.y) +
  scale_fill_gradient(low="lightblue",high="darkred",trans="sqrt",breaks = c(1,100,200,400,600,1000), labels = c(1,100,200,400,600,1000)) +
  facet_grid(.~transformation,scales = "free_x")
ggsave("../results/VST_evaluation_KO_dots.pdf",p1,width=6,height=4)
ggsave("../results/VST_evaluation_KO_hex.pdf",p2,width=6,height=4)
...
metaG.norm.match.log2<-metaG.norm.match.log2-apply(metaG.norm.match.log2[,which(colnames(metaG.norm.match.log2) %in% cog.10MGs$V2)],1,median)
metaT.norm.match.log2<-metaT.norm.match.log2-apply(metaG.norm.match.log2[,which(colnames(metaT.norm.match.log2) %in% cog.10MGs$V2)],1,median)
...
write.table(env.mat.metaG,file="../data/processed/KO_env.mat.metaG.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(env.mat.metaT,file="../data/processed/KO_env.mat.metaT.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(env.mat.match,file="../data/processed/KO_env.mat.match.txt",sep="\t",row.names=T,col.names=NA,quote=F)

write.table(metaG,file = "../data/processed/KO_metaG.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaG.match,file = "../data/processed/KO_metaG.match.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaG.norm,file = "../data/processed/KO_metaG.norm.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaG.norm.match,file = "../data/processed/KO_metaG.norm.match.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaG.norm.match.log2,file = "../data/processed/KO_metaG.norm.match.log2.txt",sep="\t",row.names=T,col.names=NA,quote=F)

write.table(metaT,file = "../data/processed/KO_metaT.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaT.match,file = "../data/processed/KO_metaT.match.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaT.norm,file = "../data/processed/KO_metaT.norm.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaT.norm.match,file = "../data/processed/KO_metaT.norm.match.txt",sep="\t",row.names=T,col.names=NA,quote=F)
write.table(metaT.norm.match.log2,file = "../data/processed/KO_metaT.norm.match.log2.txt",sep="\t",row.names=T,col.names=NA,quote=F)

write.table(ratio.mat,file = "../data/processed/KO_ratio.mat.txt",sep="\t",row.names=T,col.names=NA,quote=F)
...
#write.biom(metaG.norm.int,file="../data/processed/KO_metaG.norm.int.biom")
#metaG.norm.match.int<-occ.reduce(to.integer(metaG.norm.match))
#write.biom(metaG.norm.match.int,file="../data/processed/KO_metaG.norm.match.int.biom")
#metaT.norm.int<-occ.reduce(to.integer(metaT.norm))
#write.biom(metaT.norm.int,file="../data/processed/KO_metaT.norm.int.biom")
#metaT.norm.match.int<-occ.reduce(to.integer(metaT.norm.match))
#write.biom(metaT.norm.match.int,file="../data/processed/KO_metaT.norm.match.int.biom")

#ratio.mat.int<-occ.reduce(to.integer(ratio.mat+abs(min(ratio.mat))))
#write.biom(ratio.mat.int,file="../data/processed/KO_ratio.mat.int.biom")
