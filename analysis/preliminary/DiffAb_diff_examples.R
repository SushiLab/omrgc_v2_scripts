...
g2<-ggplot(cbind(metaT.norm.match.log2,env.mat.match),aes(x=K02703,y=K01601)) +
  geom_point(col="gray60") +
...
g3<-ggplot(cbind(metaT.norm.match.log2,env.mat.match),aes(x=K02703,y=K01602)) +
  geom_point(col="gray60") +
  geom_smooth(method="lm",se=F,col="black") +
  theme_bw() +
  xlab(str_wrap(paste(sel.genes$Enzyme[grep("K02703",sel.genes$Enzyme)],"transcript abundance"),40)) +
...
all.gath.nog<-bind_rows(ab.gath.nog,exp.gath.nog,trans.gath.nog) %>%
  ...
  select(Barcode,dmsp.synt=value) %>%
  unique()

tmp.df<-left_join(all.gath,all.gath.nog,by="Barcode")


g1<-ggplot(tmp.df %>% filter(KO %in% c("K00956","K00957","K00390","K00381","K00380") & epi=="EPI" & data_type=="Expression"),aes(x=dmsp.synt,y=value)) +
  geom_point(col="gray60") +
  #geom_smooth(method="lm",se=F,col="black") +
  geom_smooth(method="loess",linetype=2,se=F,col="black",span=0.6) +
  facet_grid(str_wrap(Enzyme,10)~.,scales = "free_y") +
  theme_bw() +
  theme(strip.text.y = element_text(angle=0)) +
  xlab("dsyB gene expression") +
  ylab("Assim. sulfate reduction genes' expression")
...
cor.test(tmp.df$dmsp.synt[tmp.df$KO=="K12339" & tmp.df$data_type=="Expression"],tmp.df$value[tmp.df$KO=="K00380" & tmp.df$data_type=="Expression"])
...
g2<-ggplot(test.df %>% filter(KO %in% c("K00956","K00957","K00390","K00381","K00380") & epi=="EPI" & data_type=="Expression"),aes(x=Euk,y=value)) +
  geom_point(col="gray60") +
  #geom_smooth(method="lm",se=F,col="black") +
  geom_smooth(method="loess",linetype=2,se=F,col="black",span=0.6) +
  facet_grid(str_wrap(Enzyme,10)~.,scales = "free") +
  theme_bw() +
  theme(strip.text.y = element_text(angle=0)) +
...
g3<-ggplot(test.df %>% filter(KO %in% c("K00956","K00957","K00390","K00381","K00380") & epi=="EPI" & data_type=="Expression"),aes(x=ChlorophyllA,y=value)) +
  geom_point(col="gray60") +
  #geom_smooth(method="lm",se=F,col="black") +
  geom_smooth(method="loess",linetype=2,se=F,col="black",span=0.6) +
  facet_grid(str_wrap(Enzyme,10)~.,scales = "free") +
  theme_bw() +
  theme(strip.text.y = element_text(angle=0)) +
  xlab("Chlrophyll a concentration") +
  ylab("Assim. sulfate reduction genes' expression") +
  scale_x_sqrt()
g1 | g2 | g3
ggsave("../results/SFig_DMSP.pdf",width=10,height=9)


tmp.df<-test.df %>%
  filter(KO %in% c("K00956","K00957","K00390","K00381","K00380") & epi=="EPI" & data_type=="Expression") %>%
  select(Barcode,Euk,ChlorophyllA,KO,value) %>%
...
cor.test(sqrt(tmp.df$ChlorophyllA),tmp.df[,4])
cor.test(sqrt(tmp.df$ChlorophyllA),tmp.df[,5])
cor.test(sqrt(tmp.df$ChlorophyllA),tmp.df[,6])
cor.test(sqrt(tmp.df$ChlorophyllA),tmp.df[,7])
cor.test(sqrt(tmp.df$ChlorophyllA),tmp.df[,8])
...
g1<-ggplot(cbind(env.mat.match,metaG.norm.match) %>% filter(epi=="EPI"),aes(x=Latitude,y=K02586,col=polar)) +
  geom_point() +
  geom_smooth(method="loess",col="gray",se=F) +
  #scale_x_sqrt(breaks=c(0,0.1,1,2,5,10,20,30,40)) +
  theme_bw() +
  scale_color_manual(values=c("#FB8072FF","#8DD3C7FF")) +
  xlab("Latitude") +
  ylab("nifD expression")
g2<-ggplot(cbind(env.mat.match,metaG.norm.match) %>% filter(epi=="EPI"),aes(x=Latitude,y=K02588,col=polar)) +
  geom_point() +
  geom_smooth(method="loess",col="gray",se=F) +
  #scale_x_sqrt(breaks=c(0,0.1,1,2,5,10,20,30,40)) +
  theme_bw() +
